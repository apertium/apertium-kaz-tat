...
#cat $CORPUS | apertium -d ../../ tr-ky-morph | apertium-tagger -p -g ../../tr-ky.prob > /tmp/tr-ky.crp.tagged;
for i in `cat $INFILE | sed 's/^ *//g' | sed 's/ /:/g' | cut -f1 -d'$'  | sed 's/ /_/g'`; do 
	# Pull out variables
	FREQ=`echo $i | cut -f1 -d':'`;
	TAGS=`echo $i | cut -f2- -d':' | cut -f2 -d'@' | cut -f2- -d'<' | sed 's/^/</g' | sed 's/<[0-9A-Za-z]\+>/<s n="&"\/>/g' | sed 's/>\?"<\?/"/g'`;
	POS=`echo $i | cut -f2- -d':' | cut -f2 -d'@' | cut -f2 -d'<' | sed 's/^/</g' | sed 's/<[0-9A-Za-z]\+>/<s n="&"\/>/g' | sed 's/>\?"<\?/"/g'`;
	LEMAPOS=`echo $i | cut -f2 -d'@' | cut -f1 -d'$' | sed 's/:/ /g'`; 
	LEMA=`echo $i | cut -f2 -d'@' | cut -f1 -d'<' | sed 's/:/<b\/>/g'`; 
