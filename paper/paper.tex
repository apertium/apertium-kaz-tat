...
The bilingual lexicon currently contains 9,269 stem-to-stem correspondences and was built mostly by hand (i.e., by translating Kazakh stems unrecognized by the morphological analyzer into Tatar).  Some toponyms and other proper names were translated semi-automatically by looking up links in Wikipedia; also, some Russian loanwords common to both languages (such as автомобиль, гонорар, etc.) were added to the bilingual dictionary automatically by taking the intersection of Russian and Kazakh wordlists.
...
The output of the morphological analyzer is highly ambigious. Both Kazakh and Tatar have a lot of affixes which join the
...
\begin{table}[htbp]
  ...
  \midrule
  Development corpus      & kaz$\rightarrow$tat & 2457    & 2       & 15.19\% \\
  \bottomrule
  \end{tabular}
    \caption{Word error rate over the development corpus.}
 %\vspace{-2.5em}
...
\begin{table}[htbp]
  ...
  \toprule
   Corpus                 & Direction           & Tokens  & Unknown & WER  \\
  \midrule
  Evaluation corpus       & kaz$\rightarrow$tat & 2862    & 43      & 36.57\% \\
  \bottomrule
  \end{tabular}
    \caption{Word error rate over the evaluation corpus.}
 %\vspace{-2.5em}
    \label{table:wer-evaluation}
  \end{center}
\end{table}
