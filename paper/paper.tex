...
Within the Apertium project, work on MT systems between Turkic languages has been started (Turkish--Kyrgyz, Azeri--Turkish, Tatar--Bashkir), but the Kazakh--Tatar system described by the present study is the closest to production-ready of them.  Among these systems is a prototype Tatar--Bashkir machine translation system which was built by the authors of this paper \citep{tyerswashingtonsalimzyanbattalov12}; due to the closeness of these languages, it proved to provide high accuracy in its translations, but being a prototype system by design, had relatively low coverage.
...
Tatar has separate words for \eng{[physical] life} (гомер) and \eng{life [as a human condition]} (тормыш), whereas Kazakh only has one word (өмір).  The lexical selection rule is provided in figure~\ref{fig:lrx3} chooses the latter Tatar translation after the word рухани \eng{soul}.
...
\end{lstlisting}
\caption{A lexical selection rule that selects Tatar тормыш for Kazakh өмір when preceded by the word рухани.}
\label{fig:lrx3}
\end{figure}
...
For Tatar, a dump of articles from the Tatar Wikipedia,\footnote{\url{http://tt.wikipedia.org/}; {\smallertt ttwiki-20130205-pages-articles.xml.bz2}} the New Testament in Tatar, and content from Radio Free Europe / Radio Liberty's Tatar-language service\footnote{\url{http://www.azatlyk.org/}} from early 2007 to early 2012 were used for testing.
...
\begin{table}
  ...
  \bottomrule
  \end{tabular}
    \caption{Word error rate over two corpora. The development corpus (\texttt{devel}) was used during development 
...
The majority of errors currently are due to one of the following: mistakes and gaps in the Tatar morphophonology component, lack of transfer rules to handle some Kazakh compounds, and disambiguation errors.
